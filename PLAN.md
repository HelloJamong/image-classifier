...
`classify.py` 단일 파일에 이미지 스캔 → 해싱 → DBSCAN 클러스터링 → 미리보기 →
...
- **phash → cosine DBSCAN:** 해밍 거리 대비 스케일 불변, 0~1 정규화로 eps 직관적
...
**Description:** 해시 벡터 배열을 받아 DBSCAN으로 클러스터링 후
`groups: dict[int, list[Path]]`와 `ungrouped: list[Path]`를 반환한다.

**Acceptance criteria:**
- [ ] `DBSCAN(eps, min_samples, metric='cosine')` 적용
- [ ] 클러스터 ID → Path 리스트 딕셔너리 반환 (ID는 0-based 정수)
- [ ] 노이즈 포인트(label=-1)는 `ungrouped` 리스트에 포함
...
|---|---|---|
| cosine metric이 이진 벡터에 부적합할 수 있음 | Medium | Task 3에서 동일 이미지 복사본 테스트로 조기 검증 |
